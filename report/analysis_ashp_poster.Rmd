...
ggplot(all_scores, aes(x = score, y = apache2)) +
    geom_boxplot() 
    # geom_jitter(color = "light blue", alpha = 0.6)
```

```{r}
ggplot(all_scores, aes(x = apache2)) +
...
# ggplot(df, aes(x = apache2, y = manual)) +
#     geom_jitter(aes(color = score), alpha = 0.6) 
...
mw_pair <- all_scores %>%
    ...
    dmap_at("score", ~fct_relevel(.x, c("manual", "ek", "ek_drg"))) %>%
    do(tidy(pairwise.wilcox.test(.$apache2, .$score, p.adj = "BH", paired = TRUE, exact = FALSE)))
```
...
knitr::kable(glance(lm, fit), digits = 4)
```
...
mod <- lm(manual ~ apache2, data = mod_df)
...
ggplot(df, aes(x = apache3, y = manual)) +
    geom_point() +
    # geom_smooth() +
    facet_wrap(~ score)
```
...
ggplot(df, aes(x = saps2, y = manual)) +
    geom_point() +
    # geom_smooth() +
    facet_wrap(~ score)
```
