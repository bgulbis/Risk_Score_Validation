...
* Comparison of scores
    ...
    - To account for performing multiple comparisons, a BH (Benjamini and Hochberg) correction is applied to the p-values

```{r}
pt <- all_scores %>%
    do(tidy(pairwise.t.test(.$apache2, .$score, paired = TRUE, p.adj = "BH")))

pairwise.t.test(all_scores$apache2, all_scores$score, paired = TRUE, p.adj = "BH")
...
mw <- all_scores[, -1] %>%
    ...
    ungroup() %>%
    mutate(bonf.adj = p.adjust(p.value, "bonferroni"),
           bh.adj = p.adjust(p.value, "BH")) %>%
    select(score:p.value, bonf.adj, bh.adj, everything())
